...
define MONTH       01; //    1 Month    January (Jan=1 to Dec=12)
define DATE      0001; // 0001 day/Date (Mon=0 to Sun=6)/(Date=1 to 28/29/30/31)
...
setup:		cli						;
			...
			ldcv	c1,127			;
			xor		r1,r1			;
			ldmam	m3				; // extrernal RAM R/W, m2 or m3
...
sw_irq1:	rti						;  
   ...
   // pjump: dw test_name; // default test_name = myapp
   //
myapp:		jsr		testinc			;
			uport	f1              ;
			uport	f2				;
...
myapp3:		cmprv	r1,3			;
			...
			xor		R1,R1			;
			str		r1,myled		;
myappend:  	ldpv	pD,tempvar1		; // restore pointer
			rts						; // uses the push return address
			
			// Application code
testinc:	ldr		r1,mycounter	;
			ldcv	c1,31			; // write 32 values
testinc2:	strpi	r1,pD			;
			inc		r1				;
			dcjnz	c1,testinc2		;
         	str		r1,mycounter	; // save last count
			rts						;
...
version_m:	dt "SW:040522-01 MasterTemplateFull";
...
mycounter:	dw 0			; // application counter	
myled:		dw 0			; // application counter
...
tmr1year:	dw	0				; // yyyy : timer 12 M = 1Y
tmr1month:	dw	0				; // 00mm : timer 28/29/30/31 D = 1M
tmr1date:	dw	0				; // ddDD : timer 24 H = 1D, dd = 0 to 6 (MON to SUN)
tmr1hour:	dw	0				; // 00hh : timer 60 m = 1H
tmr1min:	dw	0				; // 00mm : timer 60 s = 1m
tmr1sec:	dw	0				; // 00ss : timer 1S
			dw	0				; // gap			
			dw	0				; // gap
